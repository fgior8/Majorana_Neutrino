...
SignalPlots::SignalPlots(TString name) {
  ...
  h_MET2overST =     new TH1F("h_MET2overST_"    + name,"MET^{2} over ST",200,0,1000);
  h_METoverST =      new TH1F("h_METoverST_"     + name,"MET over #sqrt{ST}",200,0,1000);
...
void SignalPlots::Fill(UInt_t numberVertices, Double_t MET, Double_t MET_phi, std::vector<Lepton>& leptons, std::vector<Jet>& jets, Double_t weight, Int_t channel, Int_t cut) {
  ...
  h_MET2overST->Fill(pow(MET,2)/ST,weight);
  h_METoverST->Fill(MET/sqrt(ST),weight);
